...
const ProfileCard: React.FC = () => {
  ...
  const [profilePic, setProfilePic] = useState<string>(profileImage);
    ...
    if (isAuthenticated && userId) {
      const fetchUserData = async () => {
        try {
          ...
          setProfilePic(response.data.data.profilePicture || profileImage); // Set profile picture from API
        } catch (error) {
  ...
  const handleProfilePicUpdate = async () => {
    if (selectedFile && userId) {
      ...
      try {
        const response = await axiosInstance.put(`/account/${userId}/profile-picture`, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
        setProfilePic(response.data.profilePicture); // Update profile picture with response
