...
const ShipmentsBarChart: React.FC = () => {
  ...
  const chartData = labels.map((label, index) => {
    const dataObject: { [key: string]: any } = { name: label };
  ...
  const colors = ['#8884d8', '#82ca9d', '#ffc658', '#ff7f50', '#87cefa', '#da70d6']; // Example colors for series

  return (
    <div className="bg-light-grey p-4 sm:p-6 md:p-4 md:pb-8 rounded-lg h-64 sm:h-80 md:h-96 shadow-lg">
      <h3 className="text-xl sm:text-2xl font-medium mb-4 border-b-2 border-secondary lg:pb-2">Shipments Over Time (Bar Chart)</h3>
