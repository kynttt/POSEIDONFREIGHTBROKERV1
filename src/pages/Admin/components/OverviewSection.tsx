...
const OverviewSection: React.FC = () => {
    ...
    useEffect(() => {
        async function loadBookings() {
            ...
            const inTransit = 100 + bookings.filter((booking: any) => booking.status === 'In Transit').length; // Add real data to the starting value
            const delivered = 10000 + bookings.filter((booking: any) => booking.status === 'Delivered').length;
            const revenue = bookings.reduce((acc: number, booking: any) => acc + (booking.quote?.price || 0), 0);
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:gap-8  md:mb-8">
            <div className="bg-light-grey py-4 p-4 md:px-10 rounded-lg shadow-lg">
                ...
                <div>
                    ...
                    <p className="text-2xl">{data.totalShipments.toLocaleString()}</p> {/* Add commas */}
                </div>
            </div>
            <div className="bg-light-grey py-4 p-4 md:px-10 rounded-lg shadow-lg">
                <div className="flex justify-start">
                ...
                <div>
                    ...
                    <p className="text-2xl">{data.inTransit.toLocaleString()}</p> {/* Add commas */}
                </div>
            </div>
            <div className="bg-light-grey py-4 p-4 md:px-10 rounded-lg shadow-lg">
                <div className="flex justify-start">
                ...
                <div>
                    ...
                    <p className="text-2xl">$ {data.revenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p> {/* Format revenue with commas and 2 decimal places */}
                </div>
            </div>
            <div className="bg-light-grey py-4 p-4 md:px-10 rounded-lg shadow-lg">
                <div className="flex justify-start">
                ...
                <div>
                    ...
                    <p className="text-2xl">{data.delivered.toLocaleString()}</p> {/* Add commas */}
                </div>
            </div>
        </div>
    );
