...
const AboutUs: React.FC = () => {
  ...
  return (
    <div className="bg-[#eaeefa] flex flex-col lg:flex-row items-center lg:items-center px-4 md:px-8 lg:px-20 xl:px-32">
      {/* Image Section */}
      <div
        className={`w-full lg:w-2/3 flex justify-end items-center mb-10 lg:order-last transition-transform duration-1000 ${imageInView ? 'transform-none opacity-100' : 'transform translate-x-20 opacity-0'
          }`}
        ref={imageRef}
      >
        <img
      ...
      <div
        className={`w-full lg:w-1/3 flex flex-col justify-center text-[#252F70] max-w-prose lg:ml-2px lg:mt-16 mb-5 transition-transform duration-1000 ${textInView ? 'transform-none opacity-100' : 'transform -translate-x-20 opacity-0'
          }`}
        ref={textRef}
      >
