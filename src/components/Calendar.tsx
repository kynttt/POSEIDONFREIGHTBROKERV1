...
const Calendar: React.FC<CalendarProps> = ({ value, onChange, className }) => {
  ...
  const handleDateClick = (day: number | null) => {
    if (day !== null) {
      ...
      // ! SINCE THE APP IS INTERNATIONAL BASED, WE NEED TO USE UTC DATE
      const selected = dayjs(
        new Date(Date.UTC(currentDate.year(), currentDate.month(), day))
      );
  ...
  return (
    <div
      className={`w-full text-primary max-w-3xl h-auto bg-white shadow-2xl p-4 rounded-lg ${className}`}
    >
      ...
      <div className="grid grid-cols-7 gap-2 p-2">
        ...
        {daysArray.map((day, index) => (
          <div
            key={index}
            className={`text-center flex justify-center items-center h-16 p-2 rounded-md cursor-pointer text-secondary ${
              day
                ? isSameDay(
                    dayjs(
